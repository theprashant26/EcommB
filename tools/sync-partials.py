...
def with_preloads(page, html):
    ...
    links = "\n".join(f'<link rel="modulepreload" href="{DIST}{d}">' for d in module_deps(ENTRY[page]))
...
def with_hero_images(page, html):
    ...
    for src, name in [("assets/products/larrive/larrive-campaign.webp", "heroA"), ("assets/hero/hero-origin.webp", "heroB")]:
        ...
        html = re.sub(rf'(<img class="hcard-img" src="{re.escape(src)}")(?: srcset="[^"]*" sizes="[^"]*")?', lambda m: m.group(1) + attrs, html)
        if name == "heroA":
