...
import { esc, imageSize } from "./format.js";
...
export function plinthSetHTML(products, { eager = false, fallback = "" } = {}) {
  const prods = products.map((p) => {
    ...
    return `<span class="room-prod" style="--k:${p.cardScale || 1}"><img src="${esc(src)}" alt="${esc(p.fullName)}" width="${w}" height="${h}" ${eager ? 'fetchpriority="high"' : 'loading="lazy"'} decoding="async"><span class="room-shadow" aria-hidden="true"></span></span>`;
  }).join("");
  return `
    <div class="room-set">
